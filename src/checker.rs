...
use crate::standart_lib::{bool_op, int_float_op, num_op, str_int_op, str_op};
use crate::{
    parser::{
        self, BinaryOp, EnumDef, EnumVariant, Expr, FieldDef, FunctionDef, Item, Parameter,
        Program, Stmt, UnaryOp, ValueDecl, Visibility,
    },
...
impl Checker {
    ...
    fn check_top(&mut self) -> Result<checkReturn, CheckerError> {
        for (a, f) in self.functions.clone() {
            let cf = match self.check_func(&f, "@Program@".to_string())? {
        ...
        match v {
            ...
            Value::Expr(e) => match (**e).clone() {
                ...
                Expression::Operation { left, op, right } => operations(
                    &op,
                    &self.get_type(&left, localvars, methods)?,
                    &self.get_type(&right, localvars, methods)?,
                ),
                Expression::Call { callee, args } => match self.functions.get(&callee) {
                    Some(f) => {
                        let argtypes:Result<Vec<Type>,CheckerError> = args.iter().map(
                                |v| self.get_type(v, &localvars.clone(), &methods.clone())
                            ).collect();
                ...
                Expression::Member { object: _, member } => {
                    self.get_type(&member, localvars, methods)
                }
                Expression::MemberCall {
                    object: _,
                    callee,
                    ...
                    self.get_type(&callee, localvars, methods)
                },
            },
    ...
    fn check_comp(&mut self, c: &Comp, pf: String) -> Result<checkReturn, CheckerError> {
        ...
        for (a, f) in c.impls.clone() {
            let cf = match self.check_func(&f, pf.clone())? {
                checkReturn::Func(f) => f,
                _ => Err(CheckerError::SomethingDoesntLineup {
                    msg: "expected Func from self.check_func()".to_string(),
                })?,
            ...
            Value::Expr(e) => match (**e).clone() {
                ...
                Expression::MemberCall {
                    ...
                    match self.calls.get_mut(&o) {
                        ...
                        None => {
                            self.calls.insert(f.clone(), vec![(c, args)]);
                        }
                    }
                }
                _ => println!("not recorded {:?}",v),
            },
            _ => (),
        }
    } 

    fn check_functions_args(&self,f:&Func,args:Vec<Type>) -> Result<Type,CheckerError> {
        for ((_,exp),found) in f.args.iter().zip(args) {
            if exp.var_type == Type::Unknown {}
            else if exp.var_type != found {
                return Err(CheckerError::ValueError { exp: exp.var_type.clone(), found: found.clone() })
            }
        }
        Ok(Type::Null)
    }
